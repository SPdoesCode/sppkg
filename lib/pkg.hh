...
namespace pkg {
    ...
    inline pkgdef::Package parsePkg(std::string filepath);
    inline void installPkg(pkgdef::Package pkg);
    inline void unstallPkg(pkgdef::Package pkg);

    inline pkgdef::Package parsePkg(std::string pkgname) { // Parse the PKG file and return a struct with the data
        pkgdef::Package pkg;
